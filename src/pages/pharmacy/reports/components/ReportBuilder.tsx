...
export function ReportBuilder({ categories = [] }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      >
        <Filter size={15} className={hasFilters ? 'text-teal-600' : 'text-gray-400'} />
        <span>أدوات الفلترة</span>
        {hasFilters && (
          <span className="ms-1 px-1.5 py-0.5 text-[10px] font-bold bg-teal-600 text-white rounded-full">نشط</span>
        )}
          ...
          {categories.length > 0 && (
            <div>
              ...
              <select value={category} onChange={e => set('category', e.target.value)}
                className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-teal-400 bg-white">
