...
import { analyticsApi, type CategorySalesRow } from '../../../../api/analytics.api'
...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  ...
  return dir === 'asc' ? <ChevronUp size={12} className="text-cyan-600" /> : <ChevronDown size={12} className="text-cyan-600" />
}
        ...
        {Array.from({ length: Math.min(pages, 7) }, (_, i) => {
          ...
          return (
            <button key={p} onClick={() => onChange(p)}
              className={`w-7 h-7 rounded-lg text-xs font-medium transition-colors ${p === page ? 'bg-cyan-600 text-white' : 'hover:bg-gray-100 text-gray-600'}`}>
              {p}
            </button>
          )
        })}
...
export default function ProfitabilityByCategoryPage() {
  ...
  const { data: queryData, isLoading, refetch } = useQuery({
    queryKey: ['profitability-by-category', dateFrom, dateTo, category],
  ...
  // ── Cell formatter ────────────────────────────────────────────────────────
  function fmtCell(row: CategorySalesRow, key: ColKey): string {
    switch (key) {
  ...
  function handleExport() {
    ...
    downloadCsv(`profitability-by-category-${dateFrom}-${dateTo}.csv`, headers, rowData)
  }

  return (
    <DomainShell
      icon={Layers} iconColor="text-cyan-600" iconBg="bg-cyan-50"
      title="الربحية حسب الفئة"
      subtitle="ربحية كل فئة دوائية — مقارنة الإيرادات والتكاليف وهامش الربح بين الفئات"
    >
    <div className="space-y-5 pb-8" dir="rtl">

      {/* ── KPI cards ── */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <KpiCard label="إجمالي الفواتير"   value={totals.invoiceCount}  formatFn={fmtN} icon={ShoppingCart}  accent="border-l-cyan-400" />
        ...
        <KpiCard label="إجمالي الربح"      value={totals.grossMargin}   formatFn={fmt}  icon={Layers}       accent="border-l-cyan-600" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex flex-wrap items-center gap-2">
          ...
          {PRESETS.map(p => (
            <button key={p.key} onClick={() => setPreset(p.key)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${
                preset === p.key ? 'bg-white text-cyan-700 shadow font-semibold' : 'text-gray-500 hover:text-gray-700'
              }`}>
              {p.label}
            </button>
          ))}
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          {category && (
            <button onClick={() => setCategory('')} className="absolute left-3 top-1/2 -translate-y-1/2">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="p-4 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              <h3 className="text-sm font-semibold text-gray-700">تفاصيل الربحية حسب الفئة</h3>
              ...
              <div className="relative">
                <button onClick={() => setColPickerOpen(v => !v)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${
                    colPickerOpen ? 'bg-cyan-50 border-cyan-300 text-cyan-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
                  }`}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm" dir="rtl">
              ...
              <tbody>
                {pagedRows.map((row, i) => (
                  <tr key={`${row.category}-${row.saleDate}-${i}`}
                    className={`border-t border-gray-50 hover:bg-cyan-50/30 transition-colors ${i % 2 === 1 ? 'bg-gray-50/30' : ''}`}>
                    {displayCols.map(col => (
                      <td key={col.key}
                        className={`px-3 py-2.5 text-right whitespace-nowrap text-xs ${
                          col.key === 'category'       ? 'font-medium text-gray-900' :
                ...
                {page === Math.ceil(sortedRows.length / PAGE_SIZE) || Math.ceil(sortedRows.length / PAGE_SIZE) <= 1 ? (
                  <tr className="border-t-2 border-cyan-200 bg-cyan-50/60 font-semibold">
                    {displayCols.map((col, ci) => (
                      <td key={col.key}
                        className={`px-3 py-3 text-right text-xs whitespace-nowrap ${
                          ci === 0 ? 'font-bold text-cyan-800' :
                          col.key === 'netSales' || col.key === 'grossMargin' ? 'font-bold text-cyan-900' :
