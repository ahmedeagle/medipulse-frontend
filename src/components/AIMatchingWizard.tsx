...
const SIGNALS = [
  { icon: Barcode,       label: 'الباركود',      desc: 'تطابق دقيق للباركود = ربط فوري' },
  { icon: CheckCircle2,  label: 'اسم المنتج',    desc: 'تشابه ذكي عربي/إنجليزي + معالجة التشكيل' },
  { icon: Building2,     label: 'المُصنّع',       desc: 'مطابقة الشركة المصنّعة' },
  { icon: FlaskConical,  label: 'التركيز',       desc: '500mg, 1g, 5%, 10ml…' },
  { icon: Pill,          label: 'الشكل الصيدلاني', desc: 'أقراص، شراب، كريم، حقن…' },
]
...
export function AIMatchingWizard({ isOpen, onClose, onConfirm, isPending, unlinkedCount }: AIMatchingWizardProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-emerald-950/50 backdrop-blur-sm" onClick={!isPending ? onClose : undefined} />
      <div className="relative w-full max-w-3xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden">
        {/* Animated header */}
        <div className="relative bg-gradient-to-br from-emerald-600 via-emerald-500 to-teal-600 p-6 text-white overflow-hidden">
          {/* Glow orbs */}
          <div className="absolute -top-10 -end-10 w-40 h-40 bg-white/20 rounded-full blur-3xl animate-pulse" />
          <div className="absolute -bottom-10 -start-10 w-32 h-32 bg-emerald-300/30 rounded-full blur-3xl" />
          ...
          {step === 'intro' ? (
            ...
            <div className="space-y-5">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 max-w-md mx-auto leading-relaxed">
                  سيقوم الذكاء الاصطناعي بفحص <span className="font-bold text-emerald-700">{unlinkedCount} منتج</span> غير مربوط في مخزونك ومقارنتها بالكتالوج المركزي.
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200">
                  <div className="p-2 bg-emerald-100 rounded-lg w-fit mb-2"><Database size={18} className="text-emerald-700" /></div>
                  <p className="font-bold text-emerald-900 text-sm">1. الفحص</p>
                  <p className="text-xs text-emerald-800 mt-1 leading-relaxed">يحلل كل منتج (الاسم، الباركود، المُصنّع…)</p>
                </div>
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200">
                  <div className="p-2 bg-emerald-100 rounded-lg w-fit mb-2"><Target size={18} className="text-emerald-700" /></div>
                  <p className="font-bold text-emerald-900 text-sm">2. الترشيح</p>
                  <p className="text-xs text-emerald-800 mt-1 leading-relaxed">يحسب درجة ثقة (0-100) لكل احتمال</p>
                </div>
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200">
                  <div className="p-2 bg-emerald-100 rounded-lg w-fit mb-2"><ShieldCheck size={18} className="text-emerald-700" /></div>
                  <p className="font-bold text-emerald-900 text-sm">3. القرار</p>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {SIGNALS.map(({ icon: Icon, label, desc }) => (
                  <div key={label} className="flex items-center gap-3 p-3 rounded-xl border bg-emerald-50 border-emerald-200">
                    <div className="p-2 bg-white rounded-lg shadow-sm text-emerald-700"><Icon size={16} /></div>
                    <div className="flex-1">
                      <p className="font-bold text-sm text-emerald-700">{label}</p>
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200">
                ...
                <div className="space-y-1.5 text-xs text-gray-700">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="w-16 px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-700 font-bold text-center">≥70٪</span>
            ...
            <div className="space-y-4">
              <div className="text-center py-2">
                <div className="inline-flex p-3 bg-emerald-100 rounded-2xl mb-3">
                  <Sparkles size={28} className="text-emerald-600" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">جاهز للتشغيل؟</h3>
                <p className="text-sm text-gray-500 max-w-md mx-auto leading-relaxed">
                  اضغط <span className="font-bold text-emerald-700">"تشغيل الذكاء الاصطناعي"</span> لبدء التحليل.
              ...
              <div className="p-4 rounded-2xl bg-emerald-50 border-2 border-emerald-200">
                <p className="text-xs font-bold text-emerald-900 uppercase tracking-wider mb-2">ملخص التشغيل</p>
                <div className="space-y-1.5 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">منتجات سيتم تحليلها</span>
                    <span className="font-bold text-emerald-700">{unlinkedCount}</span>
                  </div>
                  <div className="flex items-center justify-between">
              ...
              <div className="p-3 rounded-xl bg-gray-50 border border-gray-200 flex gap-2">
                <Eye size={16} className="text-gray-500 shrink-0 mt-0.5" />
          ...
          <div className="flex items-center gap-2">
            {step !== 'confirm' ? (
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl">
                التالي
              </button>
            ) : (
              <button
                disabled={isPending}
                onClick={onConfirm}
                className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-bold bg-emerald-600 hover:bg-emerald-700 disabled:opacity-70 text-white rounded-xl shadow-md">
                {isPending
